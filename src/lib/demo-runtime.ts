...
// `window.gt`, so the landing page can offer a "try it" demo when Beans is opened
// outside General Text (no real runtime injected).
//
// It implements just the GtApi surface Beans uses (see src/gt.d.ts), backed by
// plain strings in memory and mirrored to localStorage so edits survive a refresh.
// No CRDT, no network — single-user, local, throwaway sample data. `mode: 'demo'`
// makes the data layer seed the sample ledger on first run; `version: 'demo'` lets
// the app show a "Demo" badge. Inert under a real host.

const PREFIX = 'beans-demo:'
